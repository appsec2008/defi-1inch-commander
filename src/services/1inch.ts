...
async function fetch1inch(path: string, version: string = 'v1.2') {
...
export async function getPortfolio(address: string) {
    return fetch1inch(`/portfolio/v1.0/${address}`);
}

export async function getHistory(address: string) {
    return fetch1inch(`/history/v5.0/1/events/${address}`);
}

export async function getLiquiditySources() {
    return fetch1inch(`/token/v1.2/liquidity-sources`);
}

export async function getPresets() {
    return fetch1inch(`/token/v1.2/presets`);
}
